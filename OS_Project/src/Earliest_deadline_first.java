...
public class Earliest_deadline_first 
{
	public static void main(String [] args)
	{
		...
		try
		{ 
			FileOutputStream fout=new FileOutputStream("Log_earliest_deadline_first.txt",true);
			System.out.println();
			System.out.println("We are using Earliest Deadline first real time scheduling algorithm");  
			System.out.println();
			...
			s = "\nWe are using Earliest Deadline first real time scheduling algorithm\n";
		    b=s.getBytes();  
		    fout.write(b);
			...
			for(int j=0; j<=no_of_process-1; j++)
			{
				for(int i=0; i<=no_of_process-1; i++)
				{
					if(i==j)
						continue;
					else if(((2*process_detail[j].burst_t) + process_detail[i].burst_t) > (process_detail[j].period + process_detail[j].deadline))
					{
			...
			int last_value = 0;
			// Print the output of Project of how the Processes are executed
			if(flag == 1)
			{
				System.out.println(" ");
				...
				for(int j=no_of_process-1; j>=0; j--)
				{
					if(i >= process_detail[j].check_m && process_detail[j].no_of_times != 0 && func_check((i+process_detail[j].burst_t), j, no_of_process, process_detail))
						{
							c_in++;
							System.out.print(process_detail[j].name + "executes.");
							...
							process_detail[j].check_l = process_detail[j].check_m + process_detail[j].deadline;
				...
				JFrame frame = new JFrame("Earliest Deadline First scheduling");
