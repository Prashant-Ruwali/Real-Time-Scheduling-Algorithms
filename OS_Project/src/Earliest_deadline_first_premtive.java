...
public class Earliest_deadline_first_premtive 
{
	public static void main(String [] args)
	{
		// Declaring the variables
		int temp, Process_execution;											 
		int no_of_process, sum_of_all, flag=1;
		String s ;
		byte b[];
		
		try
		{ 
			FileOutputStream fout=new FileOutputStream("Log_earliest_deadline_first_premtive.txt", true);
			System.out.println("We are using Earliest Deadline first Premtive real time scheduling algorithm");     
			...
			s = "\nWe are using Earliest Deadline first Premtive real time scheduling algorithm\n";
		    b=s.getBytes();  
		    fout.write(b);
			...
			Process_edf process_detail[] = new Process_edf[no_of_process];
			...
			{ // Get data input in all the processes
				process_detail[i] = new Process_edf();
				...
				System.out.println("Enter the Period of Process " + process_detail[i].name );
				...
				System.out.println("Enter Deadline of Process " + process_detail[i].name );
				process_detail[i].deadline = Input_from_user.nextInt();
				s = "Period of process is : " + process_detail[i].deadline + "\n";
				b=s.getBytes();  
			    fout.write(b);
			    
				if(process_detail[i].burst_t > process_detail[i].deadline)
				{
					System.out.println("Burst time should be less than Deadline");
					flag=0;
					s = "Burst time should be less than Deadline\n";
					b=s.getBytes();  
				    fout.write(b);
					break;
				}
				process_detail[i].check_l = process_detail[i].deadline;
			}
			...
			if(flag == 1)
			{
				...
				type_of_output = Input_from_user.nextInt();
			}
			...
			for(int k=0; k<no_of_process; k++)
			{
				for(int j=k+1; j<no_of_process; j++)
				{
						if(process_detail[k].no_of_times > process_detail[j].no_of_times)
						{
		            				...
		            				temp = process_detail[k].deadline;
		            				process_detail[k].deadline = process_detail[j].deadline;
		            				process_detail[j].deadline = temp;
			...
			{
				System.out.println(" ");
				int c_in=0, c_out=0, check=0, array_count=0;
				...
				while(i<lcm_total-1)
				{
					if(i >= process_detail[Process_execution].check_m && process_detail[Process_execution].no_of_times != 0 && process_detail[Process_execution].burst_t >=0 && func_check(i, Process_execution, no_of_process, process_detail))
						{
							...
							process_detail[Process_execution].burst_t--;
							if(process_detail[Process_execution].burst_t == 0)
							{
								...
								process_detail[Process_execution].check_l = process_detail[Process_execution].check_m + process_detail[Process_execution].deadline;
					...
					// To remove the garbage value contain in c_in and C_out
					if(check < c_in)
					{
						c_out=0;
						c_in=0;
				...
				JFrame frame = new JFrame("Earliest Deadline First Premtive scheduling");
	...
	private static boolean func_check(int time, int Process_no, int no_of_process, Process_edf process_detail[]) 
	{
