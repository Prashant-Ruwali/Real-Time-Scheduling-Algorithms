...
public class RMA 
{
	...
	public static void main(String [] args)
	{
		...
		int temp, no_of_process, flag=1, Process_execution;											 
		double sum_of_all, sum_of_all_lui;
		String s ;
		byte b[];
		
		try
		{ 
			FileOutputStream fout=new FileOutputStream("Log_RMA.txt",true);
			System.out.println("We are using RMA real time scheduling algorithm"); 
			...
			for(int i=0; i<no_of_process; i++)
			{
				sum_of_all = sum_of_all + (double)process_detail[i].burst_t/process_detail[i].period;
			}
			
			// Checking with LUI Calculation of whether we can Process it or not
			sum_of_all_lui =  no_of_process * (double)(Math.pow(2,(1.0/no_of_process)) -1);
			
			if(sum_of_all_lui < sum_of_all)
			{
				System.out.println("This process will not be scheduled by RMA");
				flag=0;
				
				s = "Ratio calculated by Liu formula : "+ sum_of_all_lui  + "\n";
				b=s.getBytes();  
			    fout.write(b);
			    s = "Total ratio for processing is : " + sum_of_all + "\n";
				b=s.getBytes();  
			    fout.write(b);
			...
			if(flag == 1)
			{
				...
				JFrame frame = new JFrame("Rate Monotonic Algorithm");
