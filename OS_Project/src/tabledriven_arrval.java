...
public class tabledriven_arrval extends JFrame
{
	...
	public static void main(String [] args)
	{
		...
		try
		{ 
			FileOutputStream fout=new FileOutputStream("Log_tabledriven_arrivaltime.txt",true);
		
			System.out.println(" ");
			System.out.println("We are using Table Driven real time scheduling algorithm with different arrival time"); 
			System.out.println(" ");
			
			s = "\nWe are using Table Driven with different arrival time real time scheduling algorithm\n";
		    b=s.getBytes();  
		    fout.write(b);
			...
			{ // Get data input in all the processes
				...
				System.out.println("Enter arrival time of Process " + process_detail[i].name);
				process_detail[i].arrival_time=Input_from_user.nextInt();
				s = "Arrival time of process is : " + process_detail[i].arrival_time + "\n";
				b=s.getBytes();  
			    fout.write(b); 
			    
				System.out.println("Enter burst time of Process no " + process_detail[i].name);
				...
				process_detail[i].check_m= process_detail[i].arrival_time;
			...
			{ // Save the period in an array to calculate LCM
				period_for_lcm[i]=process_detail[i].period + process_detail[i].arrival_time;
			}
			...
			for(int i=0; i<no_of_process; i++)
			{
				process_detail[i].no_of_times = lcm_total/(process_detail[i].period +process_detail[i].arrival_time);
			}
			...
			for(int j=0; j<=no_of_process-1; j++)
			{
				for(int i=0; i<=no_of_process-1; i++)
				{
					if(i==j)
						continue;
					else if(((2*process_detail[j].burst_t) + process_detail[i].burst_t) > (2 * (process_detail[j].period + process_detail[i].arrival_time)))
					{
						...
						b=s.getBytes();  
					    fout.write(b);
					    s = "Total time available for processing is : " + (2 * (process_detail[j].period + process_detail[i].arrival_time))+ "\n";
						b=s.getBytes();  
					    fout.write(b);
			...
			if(flag == 1)
			{
				...
				JFrame frame = new JFrame("Table Driven scheduling");
